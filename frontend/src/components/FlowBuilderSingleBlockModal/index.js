...
>
  <p>Variáveis</p>
  <p><strong>{"{{firstName}}"}</strong>: Retorna o primeiro nome do contato relacionado ao ticket. Se não houver contato, retorna uma string vazia.</p>
  <p><strong>{"{{name}}"}</strong>: O nome completo do contato (ou uma string vazia se não houver contato).</p>
  <p><strong>{"{{userName}}"}</strong>: O nome do usuário associado ao ticket. Se não houver usuário, retorna uma string vazia.</p>
  <p><strong>{"{{ms}}"}</strong>: Retorna uma saudação com base no horário atual (Exemplo: "Bom Dia", "Boa Tarde").</p>
  <p><strong>{"{{protocol}}"}</strong>: Combinação de control() e o ID do ticket, criando um protocolo único.</p>
  <p><strong>{"{{date}}"}</strong>: Retorna a data atual no formato dd-mm-yyyy.</p>
  <p><strong>{"{{hour}}"}</strong>: Retorna a hora atual no formato hh:mm:ss.</p>
  <p><strong>{"{{ticket_id}}"}</strong>: O ID do ticket, se existir. Caso contrário, retorna uma string vazia.</p>
  <p><strong>{"{{queue}}"}</strong>: Retorna o nome da fila à qual o ticket está associado (caso exista).</p>
  <p><strong>{"{{connection}}"}</strong>: Retorna o nome da conexão de WhatsApp associada ao ticket.</p>
</Typography>
